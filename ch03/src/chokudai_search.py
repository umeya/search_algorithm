...
class ChokudaiSearch():
    ...
    def chokudai_search(self, BEAM_WIDTH_=1, BEAM_DEPTH_:int=4, BEAM_NUMBER_:int=2):
        ...
        # bs1 = each_turn_mazes[next_turn][idx].game_score_
        # print(f'**** {last_turn_offset=} , {idx}:{best_first_action=}, game_score={bs=}, {bs1=} **')
        ...
        try:
            ...
            for arg in args:
                ...
                if k == "MAZE_WIDTH":
                    ...
                    raise Exception()
            print(
                f'{MAZE_WIDTH_=}, {MAZE_HEIGHT_=}, {GAME_END_TURN_=}, {PRINT_STATE_=}, {BEAM_WIDTH_=}, {BEAM_DEPTH_=}, {BEAM_NUMBER_=}')
        except:
            print('args error in comandoline, the command line is written as follows')
            print(
                './chokudai_search MAZE_WIDTH=4 --MAZE_HEIGHT=3 --GAME_END_TURN=4 --PRINT_STATE=y --BEAM_WIDTH=2 --BEAM_DEPTH=4')
            sys.exit(0)
